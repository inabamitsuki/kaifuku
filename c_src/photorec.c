...
#endif
  if(params->file_found_cb)
  {
    params->file_found_cb(
	file_recovery->filename,
	(file_recovery->extension!=NULL?file_recovery->extension:""),
...
void set_filename(file_recovery_t *file_recovery, struct ph_param *params)
{
  const int broken=(params->status==STATUS_EXT2_ON_SAVE_EVERYTHING ||
      params->status==STATUS_EXT2_OFF_SAVE_EVERYTHING);
  const unsigned long int sector_number=
  ...
  if(file_recovery->extension==NULL || file_recovery->extension[0]=='\0')
  {
	...
	"%s.%u/%s/%c%07lu", params->recup_dir,
	params->dir_num, subdir, (broken?'b':'f'), sector_number);
  }
  else
  {
    subdir = file_recovery->extension;
    snprintf(file_recovery->filename, sizeof(file_recovery->filename)-1,
	"%s.%u/%s/%c%07lu.%s", params->recup_dir,
	params->dir_num, subdir, (broken?'b':'f'), sector_number,
	file_recovery->extension);
  }
  {
    char subdir_path[2048];
    snprintf(subdir_path, sizeof(subdir_path), "%s.%u/%s",
	params->recup_dir, params->dir_num, subdir);
    mkdir(subdir_path, 0775);
  }
}
