...
pstatus_t photorec_aux(struct ph_param *params, const struct ph_options *options, alloc_data_t *list_search_space)
{
...
#endif
  {
	...
	wprintw(stdscr,"Error reading sector %10lu\n",
	  ...
	  {
	    const uint64_t progress_offset = offset - params->partition->part_offset;
	    const uint64_t total = params->partition->part_size;
	    const int pct = (total > 0) ? (int)(progress_offset * 100 / total) : 0;
	    const char *cur_file = (file_recovery.file_stat != NULL) ? file_recovery.filename : "";
	    params->progress_cb(pct, cur_file, params->file_nbr, params->user_data);
	  }
